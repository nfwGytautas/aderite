...
void EditorMaterialType::generateMaterialHeader(std::ostream& os) {
	...
	os << "/*\n";
	os << " *" << " DON'T CHANGE DIRECTLY" << "\n";
	os << " *" << " This is a shader header file generated by aderite for material " << this->getHandle() << "\n";
	os << " *" << " Generated at " << std::put_time(&tm, "%Y-%m-%d %H.%M.%S") << "\n";
	os << " */" << "\n\n";
	...
	for (asset::Property* prop : this->m_properties) {
		...
		{
			properties << "#define u_"
				<< this->getHandle() << "_" << prop->getName()
				<< " u_mat_buffer_" << this->getHandle()
				<< "[" << arrayIdx << "]." << access << "\n";
			break;
		}
		default:
	...
	for (Sampler* sampler : m_samplers) {
		...
		{
			samplers << "SAMPLER2D(u_"
				<< this->getHandle() << "_" << sampler->getName()
				<< ", " << samplerIdx++ << ");\n";
			break;
		}
		case asset::SamplerType::TEXTURE_CUBE:
		{
			samplers << "SAMPLERCUBE(u_"
				<< this->getHandle() << "_" << sampler->getName()
				<< ", " << samplerIdx++ << ");\n";
			break;
		}
...
void EditorMaterialType::generateVertex(std::ostream& os) {
	...
	os << "/*\n";
	os << " *" << " DON'T CHANGE DIRECTLY" << "\n";
	os << " *" << " This is a vertex shader header file generated by aderite for material " << this->getHandle() << "\n";
	os << " *" << " Generated at " << std::put_time(&tm, "%Y-%m-%d %H.%M.%S") << "\n";
	os << " */" << "\n\n";
