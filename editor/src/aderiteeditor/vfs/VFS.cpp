...
void deserializeDirectory(const YAML::Node& node, Directory* parent) {
    for (const YAML::Node& subdirNode : node["Directories"]) {
        Directory* dir = new Directory(subdirNode["Name"].as<std::string>(), parent);
        deserializeDirectory(subdirNode, dir);
    }

    for (const YAML::Node& fileNode : node["Files"]) {
...
void VFS::remove(File* file) {
    file->getParent()->m_files.erase(std::find(file->getParent()->m_files.begin(), file->getParent()->m_files.end(), file));
