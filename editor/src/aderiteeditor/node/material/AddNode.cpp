...
AddNode::AddNode() {
    // 2 inputs
    p_inputs.push_back(new InputPin(this, "None", "Input 1"));

    p_inputs.push_back(new InputPin(this, "None", "Input 2"));

    // 1 output
    p_outputs.push_back(new OutputPin(this, "None", "Output"));
}
...
void AddNode::evaluate(compiler::GraphEvaluator* evaluator) {
    ...
    compiler::EvaluatorValue var = se->addAddInstruction(std::string(m_type), p_inputs[0]->getValue(), p_inputs[1]->getValue());
