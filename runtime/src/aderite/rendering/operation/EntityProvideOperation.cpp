...
void EntityProvideOperation::execute(PipelineState* state) {
    ...
    for (auto e : renderables) {
        ...
        if (m_lookup.find(hash) == m_lookup.end()) {
            ...
            m_drawcalls.insert(m_drawcalls.end(),
                               DrawCall {meshRenderer.MeshHandle->getVboHandle(), meshRenderer.MeshHandle->getIboHandle(),
                                         meshRenderer.MaterialHandle->getFields().Type->getShaderHandle(),
                                         meshRenderer.MaterialHandle->getFields().Type->getUniformHandle(),
                                         meshRenderer.MaterialHandle->getSamplerData(), meshRenderer.MaterialHandle->getPropertyData(),
                                         std::vector<scene::TransformComponent*>()});
...
bool EntityProvideOperation::validateEntity(scene::MetaComponent& meta, scene::MeshRendererComponent& mrenderer) {
    ...
    bool isValid = true;
    if (!bgfx::isValid(mesh->getVboHandle()) || !bgfx::isValid(mesh->getIboHandle())) {
    ...
    if (!bgfx::isValid(materialType->getShaderHandle()) || !bgfx::isValid(materialType->getUniformHandle())) {
