...
void RenderOperation::initialize() {
    ...
    ADERITE_DEBUG_SECTION(bgfx::setViewName(m_viewId, this->getName().c_str());)
}
...
void RenderOperation::execute(PipelineState* state) {
    ...
    bgfx::setViewTransform(m_viewId, glm::value_ptr(eye.ViewMatrix), glm::value_ptr(eye.ProjectionMatrix));
