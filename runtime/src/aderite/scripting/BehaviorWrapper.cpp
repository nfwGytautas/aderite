...
BehaviorWrapper::BehaviorWrapper(MonoImage* image, MonoClass* klass) : m_image(image), m_class(klass) {
    ...
    m_updateMethod = this->resolveMethod(MethodSignature {nSpace, name, "Update", {{"single"}}, false}.value());

    m_initMethod = this->resolveMethod(MethodSignature {nSpace, name, "Init", {}, false}.value());

    m_triggerEnterMethod = this->resolveMethod(MethodSignature {nSpace, name, "OnTriggerEnter", {{"Aderite.Entity"}}, false}.value());

    m_triggerLeaveMethod = this->resolveMethod(MethodSignature {nSpace, name, "OnTriggerLeave", {{"Aderite.Entity"}}, false}.value());

    m_collisionEnterMethod = this->resolveMethod(MethodSignature {nSpace, name, "OnCollisionEnter", {{"Aderite.Entity"}}, false}.value());

    m_collisionLeaveMethod = this->resolveMethod(MethodSignature {nSpace, name, "OnCollisionLeave", {{"Aderite.Entity"}}, false}.value());
    ...
    if (this->hasUpdateFunction()) {
        m_updateThunk = reinterpret_cast<UpdateThunkFn*>(mono_method_get_unmanaged_thunk(m_updateMethod));
    }

    if (this->hasTriggerEnterFunction()) {
        m_triggerEnterThunk = reinterpret_cast<TriggerThunkFn*>(mono_method_get_unmanaged_thunk(m_triggerEnterMethod));
    }

    if (this->hasTriggerLeaveFunction()) {
        m_triggerLeaveThunk = reinterpret_cast<TriggerThunkFn*>(mono_method_get_unmanaged_thunk(m_triggerLeaveMethod));
    }

    if (this->hasCollisionEnterFunction()) {
        m_collisionEnterThunk = reinterpret_cast<TriggerThunkFn*>(mono_method_get_unmanaged_thunk(m_collisionEnterMethod));
    }

    if (this->hasCollisionLeaveFunction()) {
        m_collisionLeaveThunk = reinterpret_cast<TriggerThunkFn*>(mono_method_get_unmanaged_thunk(m_collisionLeaveMethod));
    }
}
